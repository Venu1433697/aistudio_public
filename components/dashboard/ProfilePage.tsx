...
export const ProfilePage: React.FC = () => {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>, type: 'banner' | 'avatar') => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        ...
        else setAvatarImage(result);
        toast.success(`${type === 'banner' ? 'Banner' : 'Profile photo'} staged. Click Save to apply.`);
      };
  ...
  const handleSave = async () => {
    ...
    try {
        ...
        localStorage.setItem('os_banner', bannerImage || '');
        localStorage.setItem('os_avatar', avatarImage);
